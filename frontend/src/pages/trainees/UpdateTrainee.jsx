...
const UpdateTrainee = () => {
  ...
  const { id } = useParams();
  const { updateTrainee, getSingleTrainee, loading, clearError } = useTraineeContext();
  ...
  // Fetch the trainee data when component mounts
  useEffect(() => {
    const fetchTrainee = async () => {
      setFetchLoading(true);
      const result = await getSingleTrainee(id);
      if (result.data) {
        setFormData({
          ...
          ageBracket: result.data.ageBracket || '',
          enterpriseId: result.data.enterprise?._id || '', 
        });
      } else {
        // Handle case when trainee is not found
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await updateTrainee(id, formData);

    if (result.data) {
      toast.success(result.success || "Participant updated successfully!", {
        position: "top-right",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
      ...
      // Navigate back to trainees list after a short delay
      setTimeout(() => {
    ...
    <div className='mx-auto max-w-2xl text-center bg-gray-50 min-h-screen py-8'>
      <div className="bg-white p-8 rounded-2xl shadow-2xl transition-all duration-300 hover:translate-y-[-2px]">
        <div className="flex flex-col items-center justify-center gap-3 mb-8">
          <h1 className='text-3xl font-bold text-gray-900'>Update Trainee</h1>
          <p className='text-gray-600'>Update the details of this trainee</p>
        </div>
